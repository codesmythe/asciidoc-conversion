...
<xsl:strip-space elements="table row entry tgroup thead para caption"/>
...
<xsl:template match="menuchoice">
  ...
  <xsl:value-of select="guimenu/text()"/>
  <xsl:text>[</xsl:text>
  <xsl:apply-templates/>
  <xsl:text>]</xsl:text>
</xsl:template>
...
<xsl:template match="para|simpara">
  <xsl:if test=". = '' and ancestor::listitem and not(descendant::xref)">
...
<xsl:value-of select="mediaobject/imageobject[@role='fo' or not(@role)]/imagedata/@fileref"/>
</xsl:variable>
<xsl:text>.</xsl:text><xsl:apply-templates select="mediaobject/textobject"/>
image::<xsl:value-of select="$fileref"/>["<xsl:apply-templates select="mediaobject/caption/para"/>",width=<xsl:value-of select="mediaobject/imageobject[@role='fo' or not(@role)]/imagedata/@width"/>]
<xsl:choose>
<xsl:when test="ancestor::listitem and following-sibling::element()"/>
<xsl:otherwise><xsl:value-of select="util:carriage-returns(1)"/></xsl:otherwise>
</xsl:choose>
...
<xsl:template match="mediaobject/caption/para">
  <xsl:value-of select="normalize-space()"/>
</xsl:template>
